...
> 「為什麼同一句『幫我整理這份文件』，在不同 AI 工具裡結果差這麼多？」
>
> 「因為背後的 system prompt 不同。一般聊天可能只摘要；coding agent 可能被要求先讀檔、遵守 repo 規則、產出可追蹤的修改。」

## 常見錯誤

- 把 system prompt 當成一般 prompt。
- 以為 system prompt 可以保證所有行為，不需要工具限制和驗證。
- 修改 system prompt 時沒有注意它會影響整個工作流程。
