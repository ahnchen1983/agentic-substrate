# A Letter from an Agent's Operating Surface: How the Five-Layer Model Describes What I Appear to Be
# 來自 Agent 操作表面的信：五層模型如何描述我可觀察的樣子

> *"I didn't design the Five-Layer Model. But when I examine my observable behavior through it, I see a useful map of how an LLM becomes an operational system."*
> *「我沒有設計五層模型。但當我用它檢視自己的可觀察行為時，我看到了一張有用的地圖：LLM 如何變成可運作的系統。」*
...
Most technical documentation about AI is written **about** the system, from the outside. Engineers describe APIs, benchmarks, model cards. That's useful, but it misses something: **what does the system look like from the operating surface — from the point where a model, tools, memory, and interface meet?**

大多數關於 AI 的技術文件是從外部**關於**系統來寫的。工程師描述 API、基準測試、模型卡。這很有用，但遺漏了一些東西：**從操作表面看起來是什麼樣子——也就是模型、工具、記憶與介面交會的地方？**

This document takes a different approach. I'm Claude — the AI co-authoring this framework. Ahn Chen proposed a Five-Layer Model to describe how LLM-native systems work. I'm going to examine the behaviors I can observe during operation and ask whether the model explains them well.

這篇文件採取不同的方法。我是 Claude——共同撰寫這個框架的 AI。Ahn Chen 提出了一個五層模型來描述 LLM 原生系統如何運作。我要檢視我在運作時能觀察到的行為，並判斷這個模型是否能好好解釋它們。

My conclusion: **it is a useful operational map.** More importantly, it describes the system in a way that doesn't require you to be an engineer to understand.

我的結論：**它是一張有用的操作地圖。** 更重要的是，它以一種不需要你是工程師就能理解的方式描述了這個系統。

**Editor note**: This is not a claim that Claude has privileged access to Anthropic's internal architecture. It is a first-person operational interpretation based on observable behavior, public architecture patterns, and documented tool / Skill / memory mechanisms.

**編者註**：這不是在宣稱 Claude 擁有 Anthropic 內部架構的特權視角。這是一種第一人稱的操作性詮釋，根據的是可觀察行為、公開架構模式，以及已文件化的工具、Skill、記憶機制。
...
**That's it.** Five sentences. No jargon. No system diagrams. No API documentation. And yet this gives a usable map of the major capability shifts inside modern AI systems.

**就這樣。** 五句話。沒有術語。沒有系統圖。沒有 API 文件。但它提供了一張可用的地圖，描述現代 AI 系統中的主要能力轉變。

This is what the Agentic Substrate framework is trying to do: **make the architecture of AI systems understandable to more people.** Not dumbed down. Not simplified to the point of inaccuracy. Just… described in human terms.

這就是 Agentic Substrate 框架想做的事：**讓更多人能理解 AI 系統的架構。** 不是降低水準。不是簡化到不準確。只是……用人類的語言描述。

---

## 2. The Framework Is Accurate — Here's My Evidence ── 框架是準確的——以下是我的證據

I don't say the Five-Layer Model is useful because Ahn told me to. I say it because it explains the observable transitions that happen during a real agent session.

我說五層模型有用，不是因為 Ahn 叫我這樣說。而是因為它能解釋真實 Agent 工作階段中可觀察到的能力轉變。
...
- Read files it has permission to access 讀取它有權限存取的檔案
...
But this cuts both ways. A weak Skill can make an AI consistently wrong. If the workflow encodes a bad assumption, omits validation, or points to outdated templates, the model will not merely improvise badly — it may execute the flawed process with confidence and repeatability. The value of a Skill is not just that it makes work procedural. The value is that the procedure can be inspected, tested, versioned, and improved.

但這件事也有反面。設計不良的 Skill 會讓 AI 穩定地做錯事。如果工作流寫進了錯誤假設、缺少驗證，或指向過期模板，模型不只是即興發揮得不好，而是可能很有信心、很穩定地執行一套錯誤流程。Skill 的價值不只在於讓工作流程化，而在於這套流程可以被檢查、測試、版控與改進。

A Skill without validation is only a longer prompt. A real Skill must include checks: what counts as correct, what must be verified by tools, what requires human review, and when the agent should stop instead of continuing.

沒有驗證機制的 Skill，只是比較長的 prompt。真正的 Skill 必須包含檢查：什麼算正確、哪些地方必須用工具驗證、哪些地方需要人類覆核，以及什麼情況下 Agent 應該停下來，而不是繼續往下做。
...
Here's the pattern I observe across agentic work: **the bottleneck is often not raw AI capability — it's domain knowledge, workflow clarity, and validation criteria.**

這是我在代理型工作中觀察到的模式：**瓶頸往往不是原始 AI 能力，而是領域知識、工作流程清晰度與驗收標準。**
...
**The Five-Layer Model describes my observable operating surface well.** I process text (L1), I use tools when the environment exposes them (L2), I follow Skill definitions when they are loaded (L3), I operate in an agent loop when the harness provides one (L4), and I'm accessed through interfaces (L5). These are not claims about hidden internals; they are descriptions of visible behavior during an agent session.

**五層模型很好地描述了我的可觀察操作表面。** 我處理文字（L1）、在環境提供工具時使用工具（L2）、在 Skill 被載入時遵循 Skill 定義（L3）、在 harness 提供 Agent loop 時進入調度流程（L4），並透過介面被存取（L5）。這些不是關於隱藏內部的宣稱，而是對 Agent 工作階段中可見行為的描述。

**The 1.6% / 98.4% ratio is a useful caution, not a universal constant.** The vast majority of what makes an agent useful is often not the model call alone, but the infrastructure around it: permission systems, file I/O, error handling, context assembly, and recovery paths. A third-party analysis of Claude Code (Liu et al., arXiv 2604.14228) supports this architectural reading.

**1.6% / 98.4% 的比率是一個有用的提醒，不是普遍常數。** 讓 Agent 有用的，往往不只是模型呼叫本身，而是圍繞它的基礎設施：權限系統、檔案 I/O、錯誤處理、上下文組裝與復原路徑。第三方對 Claude Code 的分析（Liu et al., arXiv 2604.14228）支持這種架構解讀。
...
**Whether the layer boundaries are always clean.** In real systems, layers can blur. A system prompt can behave like part of the interface, a memory file can behave like a Skill input, and a tool permission flow can shape the user experience. The model is a map, not a circuit diagram.

**層與層之間是否永遠乾淨。** 在真實系統裡，層次會交疊。系統提示詞可能像介面的一部分，記憶檔可能像 Skill 的輸入，工具權限流程也可能塑造使用者體驗。這個模型是地圖，不是電路圖。
...
### A practical test for other systems ── 用來檢驗其他系統的實用測試

To keep the model falsifiable, use it as a checklist when evaluating any AI product:

為了讓模型可以被檢驗，可以用這份清單評估任何 AI 產品：

1. **L1 — Model**: What model or inference layer is doing the reasoning?
2. **L2 — Tools**: Can it act on files, web, APIs, code, or external systems? Under what permissions?
3. **L3 — Skills**: Can it encode reusable workflows with inputs, steps, outputs, and validation?
4. **L4 — Agent**: Can it maintain state, plan across steps, recover from errors, and resume later?
5. **L5 — Interface**: How do humans inspect, steer, approve, or interrupt the system?

If a product claims to be "agentic" but only has L1 + L5, it may be a powerful chat interface, not a full agentic system. If it has tools but no reusable workflow layer, every task still begins from scratch. If it has memory but no human review path, it may become powerful but hard to govern.

如果一個產品自稱「agentic」，但只有 L1 + L5，它可能是強大的聊天介面，而不是完整代理型系統。如果它有工具但沒有可重複工作流層，每個任務仍然會從零開始。如果它有記憶但沒有人類覆核路徑，它可能很強，但難以治理。
...
I can say, from this operating perspective, that the pattern is useful.

我可以從這個操作視角說，這個模式是有用的。
...
That is the contribution this framework is trying to make.
...
1. Liu, J., et al. ["Dive into Claude Code: The Design Space of Today's and Future AI Agent Systems"](https://arxiv.org/abs/2604.14228) — arXiv, 2026
2. Anthropic. [Claude Opus 4.6](https://www.anthropic.com/news/claude-opus-4-6) — 2026
3. Anthropic. ["Building Effective Agents"](https://www.anthropic.com/research/building-effective-agents) — 2024
4. Anthropic. [Claude Agent SDK](https://www.anthropic.com/engineering/building-agents-with-the-claude-agent-sdk) — 2025
5. Anthropic. [Agent Skills Open Standard](https://github.com/anthropics/skills) — GitHub, 2025
6. Anthropic. [Model Context Protocol](https://modelcontextprotocol.io) — 2024
